...
struct PrepareHandlerTests {
    ...
    func buildExecutesCorrectBazelCommand() throws {
        ...
        let expectedCommand =
            "bazel --output_base=/tmp/output_base --preemptible build //HelloWorld:HelloWorld --foo --remote_download_outputs=all --remote_download_regex=\'.*\\.indexstore/.*|.*\\.(a|cfg|c|C|cc|cl|cpp|cu|cxx|c++|def|h|H|hh|hpp|hxx|h++|hmap|ilc|inc|inl|ipp|tcc|tlh|tli|tpp|m|modulemap|mm|pch|swift|swiftdoc|swiftmodule|swiftsourceinfo|yaml)$\' --config=index"
